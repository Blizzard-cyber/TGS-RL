...
| 净化集成点 | `8f7494d7ee0c10bdde0a1e150e99485045d5f41d`；将验证分支整理为执行链、工具链和文档三个提交 |
...
原始报告继续绑定实际运行的 `d033566`，而不是把提交历史净化后的 `8f7494d` 伪装成
直接执行过 GPU 的 revision；后者的执行路径与验证树一致，额外差异仅为工具链和文档收尾。
...
在 HAMi H1 验证及严格 teardown 后，提交
`9c65d466831e15143b946bd7728a39736bb19ac7` 再次执行 E1 并得到 `PASSED`：

- NVIDIA Device Plugin DaemonSet `1/1`；
- Node `nvidia.com/gpu=1`；
- Node 无残留 `hami.io/*` 注解；
- Scheduler、DRA allocation 与 worker UUID 再次一致；
- baseline/variant 各完成 1 次 warmup + 3 次 measurement；
- 最终 namespace 无 JobRunBundle、Workload、Job 或 Pod 残留。

该回归证明 H1 的 GPU-manager 切换与恢复未破坏既有 Full GPU DRA 主链。
...
| `gpu-smoke/e1-full-gpu/report.json` | `5458f90e0fa0bdfb9e1a0e1dfb2fca1349814caeeab7fa867650daf017bc03c4` |
| `gpu-smoke/e1-full-gpu/artifacts/raw/archive.tar.gz` | `874d50d56f6a8a920ad1d046f2ad5724cfce7d3f9406cae8e6335a5626a31438` |
| `gpu-smoke/e1-full-gpu/artifacts/traces/variant-trace.json` | `460f9ae9e2b462602462a54f8c63feee82c3ea0045160db11989f4921cf04788` |
