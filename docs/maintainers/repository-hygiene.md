...
| Dependency locks | `go.sum`, `uv.lock`, `console/package-lock.json`, `configs/hardware/gpu-requirements.lock` | Reproducible control-plane and isolated GPU-workload dependency resolution |
...
| Deployment contracts | Dockerfiles, `compose.yaml`, `compose.gpu.yaml`, `deploy/` | Local and Kubernetes packaging |
...
| Real environment configuration | `.env`, kubeconfig, registry login files, `configs/hardware/environment.json`, `values.production.yaml` |
