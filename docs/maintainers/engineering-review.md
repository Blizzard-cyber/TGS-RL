...
| 项 | 源码/证据入口 | 影响与下一步 |
|---|---|---|
| 通用 GPU Helm 接线不完整 | `deploy/helm/tgsrl/templates/scheduler.yaml`、`values.yaml` | 默认 CPU 配置，未完整暴露 NVIDIA v2/helper/host-device/shared mount；不能只改 gpuProfile 就承诺 GPU 全栈部署。先用已支持的 GPU smoke 路径 |
| MPS 动态控制仍缺目标进程证据 | `scheduler-go/provider/nvidia/mps_backend.go`、bootstrap MPS PID 发现 | 回读 server percentage 不证明运行中的独立 worker 获得预期份额；需实机核对 server/client 归属和生效时机 |
...
| hardware cleanup 有 GET→DELETE 竞争窗口 | `scripts/hardware_environment_driver.py` cleanup | 已核对 ownership labels，但未把 UID/resourceVersion 作为 API delete precondition；多控制者环境应补条件删除 |
| 硬件 provenance 仍不完整 | 同文件 preflight、配置与 hook 加载 | 尚未完整固定 environment config、Job template、hook 文件、rendered Job 和 cluster UID；不作为最终可复现实验基线 |
| 真实故障与恢复覆盖不足 | Operator、registry、process/Kubernetes backend | 补 worker crash、callback timeout/响应丢失、组件重启、部分失败与清理证据；无 MIG 的机器不阻塞 Full/HAMi 路径 |
...
| 检查 | 结果 |
|---|---|
| Python Runtime/Storage/Governance/API | **529 passed**；文档整理期间的缺链接失败已在最终重跑关闭 |
...
| 性能回归 | Scheduler 小/大 fixture CPU P95 约 7.45/17.03 ms；Provider observation 6 µs，均在当前代码预算内；非 wall-clock SLA |
...
5. 分别补完整 trainer callbacks、MPS/可用 MIG 和通用 GPU Helm；未支持硬件标记 NOT_RUN。
