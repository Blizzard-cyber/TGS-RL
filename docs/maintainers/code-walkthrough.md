...
| 普通单测以单批本机 p95 判定性能 | CI 负载变化会造成孤立抖动失败，也不能代表生产性能 | 从普通单测移出墙钟断言；独立非 race CI 固定单 P 并使用多批次进程 CPU-time P95 中位数阻断持续回退，同时保留显式 wall-clock benchmark | `make test-performance`、`scheduler-go/scheduler/benchmark_test.go` |
