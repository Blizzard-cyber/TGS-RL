...
`13d0f34b23c581b73d32ab959d6db6bb9c83bfd0`，E5-STATIC 正式确认基线为
`9a4321d656b995c4282dbda77b872a59d528178b`；实验前工程验收基线仍为
...
静态份额共置干扰 pilot。E6 已正式通过；E5-STATIC 也已在 `9a4321d` 独立确认中
以平均干扰率 `10.63%` 通过 `32%` 门槛。`E5-STATIC` 不能替代正式 E5 的动态
share/priority 门禁。
...
| 实验平台与证据合同 | **完成** | E1–E8 campaign、scenario、runner、driver、证据哈希、校准入口和 fail-closed 规则已实现；E6 与 E5-STATIC 已完成 A10 验证 | 继续为 E3/E4/E5/E7 接入正式 workload 或 hook |
...
| E5 共置干扰隔离 | **NOT_RUN** | `E5-STATIC` 已 `PASSED`：`9a4321d` 三轮干扰率 7.17%/5.81%/18.89%，均值 10.63% ≤ 32%；正式 E5 未执行 | 正式 E5 仍需动态 share/priority 权威动作与回读 |
...
- **辅助实验线：E5-STATIC 已通过 32% 静态干扰门槛；不替代正式 E5。**
...
1. 接入完整 veRL workload，明确模型、数据、seed、batch、节点和镜像 digest，推进 E3。
2. 为 E4、正式 E5 和 E7 实现并审查环境 hook；正式 E5 不由 E5-STATIC 自动替代。
3. 单独准备 MIG 和多节点环境执行 E2、E8；资源未具备时保持 `BLOCKED/NOT_RUN`。
4. 将 E6/E5-STATIC 等原始证据转成统计表和图，完成论文实验设计、结果分析、局限性和答辩材料。
