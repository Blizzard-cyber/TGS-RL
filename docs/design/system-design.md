...
再把调度计划落实到可核对身份的资源与进程。有效更新吞吐（VUG）是设计目标，**不是默认
随系统启动即成立的收益声明**。是否完成特定硬件、性能、故障或多节点验证，以
[支持范围与限制](../reference/current-capabilities.md)、[E1-E8 验证合同](gate-e1-e8.md)和
[验证记录](../validation/README.md)为准。
