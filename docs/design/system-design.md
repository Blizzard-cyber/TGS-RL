...
已证明的收益**；系统接线、故障行为和单节点可重复运行已经完成工程验收，下一阶段是正式
workload 下的 E2–E8 实验与阈值标定。

当前工作树已把 E6 五步 lifecycle cost 接到 scoped worker registry，并提供
`E5-STATIC` 静态 HAMi 共置干扰 pilot。两者是实验实现，不是硬件结果；正式 E5 仍保留
