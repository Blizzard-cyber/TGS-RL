...
E5-STATIC 也已取得真实 A10 数据并完成独立门禁确认，下一阶段是正式 workload 下的
其余实验。
...
`32%` 门槛，并在 `9a4321d` 独立运行中以 `10.63%` 通过。正式 E5 继续保留动态
share/priority 的 Scheduler 权威动作和 infrastructure readback 要求。
