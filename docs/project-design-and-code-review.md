...
分析四组；顶部状态栏持续显示当前集群连接与协议版本；页面标题、筛选工具栏和内容工作区使用
统一栅格。总览以状态条、任务表和异常队列为主，优先显示活跃或最近更新的任务；任务中心使用
固定任务侧栏和弹性详情区，高频 lifecycle 操作常驻首屏，JSON 创建任务/回放收纳在高级操作中。
长 ID 使用等宽字体并允许安全换行，表格负责高密度比较，卡片只用于选中态、异常和摘要。
移动端将侧栏转换为可横向滚动的功能导航，表格和 Trace 轨道在各自容器内滚动，不压缩字段。
